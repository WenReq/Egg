 ...
 * @Date: 2024-03-22 11:42:18
 * @LastEditors: 温少昌 wenshaochang@huizhihuyu.com
 * @LastEditTime: 2024-03-22 15:48:37
 * @FilePath: /egg/app/controller/bill.js
 * @Description: 账单的控制器
 */
...
class BillController extends Controller {
    async add() {
        ...
        try {
            ...
            if(!decode) return
            user_id = decode.id
            // user_id 默认添加每个账单项，作为后续指定用户账单的标识。
            // 可以理解为，我登录 A 账户，那么所做的操作都得加上 A 账号的 id，后续获取的时候，就过滤出 A 账户 id 的账单信息。
    ...
    async list () {
        const { ctx, app } = this;
        // 获取，日期，分页数据，类型，这些都是我们在前端传给后端的数据
        const { page = 1, page_size = 10, type_id = 'all', date } = ctx.query
